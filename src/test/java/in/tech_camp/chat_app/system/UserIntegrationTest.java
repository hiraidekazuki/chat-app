...
public class UserIntegrationTest {
  //@Autowiredを付与してフィールド定義を行うことで、テスト実行時に自動でオブジェクトが生成される
  @Autowired
  private MockMvc mockMvc; //←生成されたオブジェクト
  ...
  public void ログインしていない状態でトップページにアクセスした場合サインインページに移動する() throws Exception {
    ...
    //作られた疑似オブジェクトにパーフォームでHTTPリクエストを送る。
   mockMvc.perform(get("/"))
   //andExpectでちゃんとページが遷移したというレスポンスが来たかを確認している。
